...
# This assumes valid URL is used and has the same structure as our above example
...
```{r}
query_api <- function(year=2022, vars = c("PWGTP"=NA, "AGEP"=NA, "SEX"=NA), geo_level = NULL, geo_code){
  ...
  # geo_level = geographic level Region, Division or State, default is all of the US which means &for is not in our URL hence the null.
  ...
  } else if (!(geo_level %in% valid_geo)){
