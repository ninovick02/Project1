...
URL_funct <-function(Census_URL){ 
 ...
 # Next convert numeric variables with similar if logic to check if they are in tibble we can loop through this
  ...
  mid_time <- function(time) {
   ...
   if (length(time) == 0 || is.na(time) || time == "NA" || time == "0") {
    return(NA)
  }
     ...
     end_parts <- strsplit(end, " ")[[1]]
     start_time <- start_parts[1]
     end_time <- end_parts[1]
  
  
     # split "x:xx" into hours and minutes
     # Note: The intervals are never split between different hours
     start_time_parts <- strsplit(start_time, ":")[[1]]
     end_time_parts <- strsplit(end_time, ":")[[1]]
     
     hours <- as.numeric(start_time_parts[1])
     
     # convert to minutes + add 2 for midpoint
     # not all intervales are 4 minutes apart (eg 007 in JWAP and 148 in JWDP are 9 minutes apart)
     minutes_start <- as.numeric(start_time_parts[2])
     minutes_end <- as.numeric(end_time_parts[2])
     minutes <- (minutes_end + minutes_start)/2
     
     # put back together
     if (minutes == round(minutes)){
       mid_pt <- paste0(hours,":", sprintf("%02.f", minutes), " ", start_parts[2])
     }
     else{ # if there are half-minutes (30 seconds)
       mid_pt <- paste0(hours,":", sprintf("%02.f", minutes), ":30",  " ", start_parts[2])
     }
 ...
 # I moved the sapply outside of tif statements bc it was interpreting the vector as factor in stead of character
  ...
  if ("JWAP" %in% colnames(Census_info)) {
    ...
    JWAP_label <- JWAP_lable[unique(Census_info$JWAP)]
    Census_info <- Census_info |>
      mutate(JWAP = factor(JWAP, levels = times_JWAP, labels = JWAP_label))
  }
  
  # JWDP
  if ("JWDP" %in% colnames(Census_info)) {
    times_JWDP <- unique(Census_info$JWDP)
    JWDP_label <- JWDP_label[unique(Census_info$JWDP)]
    Census_info <- Census_info |>
      mutate(JWDP = factor(JWDP, levels = times_JWDP, labels = JWDP_label))
  }
...
```{r}
query_api(vars = c(PWGTP = NA, JWDP = NA, SEX = 1))

```


 Last Data processing function
 Specifies multiple years of survey data
...
build_multi_year_tbl <- function(years = c(2012, 2013, 2014, 2015, 2016, 2017, 2018, 2019, 2021, 2022), vars = c("PWGTP"=NA, "AGEP"=NA, "SEX"=NA), geo_level = "state" , geo_code = 11){
  ...
  for (i in 1:length(years)){
    multi_year_tbl <- bind_rows(multi_year_tbl, query_api(years[i], vars, geo_level, geo_code))
    multi_year_tbl <- multi_year_tbl |>
      mutate(
        year = years[i],
        .before = 1
      )
...
build_multi_year_tbl(years = yrs)
```
