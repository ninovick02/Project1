...
No worries I figure we can focus on the code and clean up the explanation toward the end. I am still going to try and explain as I go so you/I don't get lost but it may not be the most poetic writing. 
...
The first step in our homework is to try and query an API. I choose a random URL to test before we get into our function writing. 
...
# May want to rename from id_info
id_info <-GET(test_URL)
parsed <- fromJSON(rawToChar(id_info$content))
# The first row is column names
Census_Info <- as_tibble(parsed[-1,])
names(Census_Info) <- parsed[1,0] 
Census_Info
```

```{r}
query_api <- function(year=2022, vars = c("PWGTP"=NA, "AGEP"=NA, "SEX"=NA), geo = c("ALL"=NA)){
