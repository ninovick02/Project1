...
```{r}
Census_tibble <- query_api(vars = c(PWGTP = NA, JWDP=100, SEX = 2))
# does our function address 001 vs 1 vs 01?
```
...
summary_Census <- function(tibble, num_vars=NULL, cat_vars=NULL){
  ...
  for (i in cat_vars){
    counts <- xtabs(weight ~ data[[v]])
    results[[v]] <- as.list(counts)
  }
...
summary_census(Census_Tibble)
```

We are now going to **test** our summary function!
